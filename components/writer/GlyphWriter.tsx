...
/**
 * Stroke-order writing practice for a single glyph: trace strokes that snap
 * into place when close enough, with order playback and a guide toggle.
 * Falls back to freehand drawing over a faint glyph when no path data exists.
 */
export function GlyphWriter({
  glyph,
  caption,
  paths,
  strokeCount,
  ...
  glyph: string;
  /** Short label rendered beside the glyph (romaji, meaning, …). */
  caption: React.ReactNode;
  /** Per-stroke SVG paths in the KanjiVG viewBox; undefined → freehand mode. */
  paths: string[] | undefined;
  /** Expected stroke count for the freehand fallback. */
  strokeCount?: number;
  /** Text for the audio button; omit to hide it. */
  audioText?: string;
  viewBox: number;
  onPrev: () => void;
  onNext: () => void;
  ...
  const expected = paths?.length ?? strokeCount;
